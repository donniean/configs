...
const promptQuestions = async ({ configParsed }) => {
  ...
  c = { ...c, ...languagesAnswers };

  const configsQuestions = getConfigsQuestions({ configParsed });
  const { configs: configsAnswers } = await prompt(configsQuestions);
  configsAnswers.forEach((key) => {
    c.configs[key] = true;
  });
  /* const configsAnswers = await prompt(configsQuestions);
  c = { ...c, ...configsAnswers }; */
  console.log(c);
  ...
  if (hasESLint) {
    const eslintQuestions = getEslintPresetQuestions({ configParsed });
...
const handleModules = async ({ configParsed }) => {
  ...
  if (prettier[0]) {
    await run('Prettier', async () => {
      // eslint-disable-next-line global-require
      await require('../prettier')({ configParsed });
    });
  }
